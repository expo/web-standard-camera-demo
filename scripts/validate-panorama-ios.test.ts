...
test('panorama validator summarizes WebXR LiDAR demo frame misses from copied logs', () => {
  const seen = parseMetricLogText(`
 LOG WEBGPU_DEMO_PROFILE {"demo":"lidar-depth-webxr","elapsedMs":2000,"frameNumber":0,"lastFrameOutcome":"depth-miss","sessionEnded":false,"viewMode":1,"viewModeLabel":"Depth","counts":{"xrCallbacks":30,"xrCallbacksPerSec":15,"depthMisses":30,"depthMissesPerSec":15,"renderFrames":30,"renderFramesPerSec":15},"timings":{"renderSubmitPresent":{"avgMs":2.5,"maxMs":7.25}}}
 LOG WEBGPU_DEMO_PROFILE {"demo":"lidar-depth-webxr","elapsedMs":2000,"frameNumber":12,"lastFrameOutcome":"uploaded","sessionEnded":false,"viewMode":1,"viewModeLabel":"Depth","counts":{"xrCallbacks":20,"xrCallbacksPerSec":10,"xrFrames":12,"xrFramesPerSec":6,"renderFrames":20,"renderFramesPerSec":10,"cameraUploadedBytes":4096},"timings":{"writeCameraTexture":{"avgMs":0.4,"maxMs":0.9},"renderSubmitPresent":{"avgMs":3,"maxMs":8}}}
 LOG WEBXR_DEMO_FRAME_ERROR {"demo":"lidar-depth-webxr","errorMessage":"undefined is not a function","errorName":"TypeError","frameNumber":12,"lastFrameOutcome":"uploaded","viewMode":1,"viewModeLabel":"Depth"}
  `);
  ...
  expect(panoramaBottleneckSummary(seen)).toContain(
    'WebGPU demo lidar-depth-webxr: view Depth, outcome uploaded, XR callbacks 50, uploads 12, render frames 50, misses pose/depth/camera 0/30/0, frame errors 0, session ended no'
  );
  expect(panoramaBottleneckSummary(seen)).toContain(
    'WebXR demo frame error: TypeError: undefined is not a function, frame 12, view Depth, last outcome uploaded'
  );
});
...
test('panorama validator distinguishes native session stops from JS keyframe rejection', () => {
  ...
  const interrupted = {
    ...
    PANORAMIC_XR_FRAME_PUMP_PROFILE: {
      ...
      lastDeliveredFrameNumber: 10,
      latestFrameNumber: 10,
      nativeSessionReason: 'ARKit scene depth session was interrupted',
