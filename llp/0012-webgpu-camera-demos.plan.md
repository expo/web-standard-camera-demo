...
The demo routes accumulate render frame rate, camera/synthetic upload counts,
uploaded bytes, native `grabFrame()` time, `writeTexture()` enqueue time, and
render submit/present timing through the shared `createWebGpuPerfProbe`
counters. Per-frame logging is intentionally off: take measurements through
device profiling tools or by reattaching a logger during local debugging
rather than from `console.log` in shipping builds.
