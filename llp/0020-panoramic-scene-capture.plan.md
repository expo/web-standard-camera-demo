...
- WebXR payload tuning: native depth payloads encode ARKit samples below the
  ...
  startup surfels. The
  CPU-visible camera image is an internal 1920x1080 BGRA preview for the surfel
  color path, improving scan color/preview fidelity without adding a
  camera-resolution option to `requestSession()`.
...
The current `LiDARDepthSource` returns display-oriented 1920x1080 BGRA camera
bytes generated by `makeCameraPreviewFrame()`. That is sufficient for surfel
color sampling and visual preview, but it is not automatically geometry-grade
texture input.
...
runtime should produce small previews with a direct aspect-fill downsample and
YCbCr-to-BGRA conversion. Full 1080p previews may use CoreImage first to avoid
running a scalar native conversion loop over every returned pixel. Physical
logs report the preview path so a slow
...
Do not assume the current CPU preview bytes alone are sufficient for texture
...
- do not retain full-resolution BGRA camera frames for every keyframe in the
  ...
  downsampled color image. The current native WebXR camera image is 1920x1080
  BGRA for preview and color fidelity, so the scan loop must avoid retaining a
  copy per keyframe.
