...
const FRAME_UPLOAD_INTERVAL_MS = 66;
...
export default function LiDARDepthScreen(): React.JSX.Element {
  ...
  // Unmount cleanup: ARKit always gets torn down and the context lock
  // released. We deliberately do not auto-start ARKit on mount — see the
    ...
    const startRender = (): void => {
      try {
        ...
        const uploadFrame = (frame: NativeLiDARDepthFrame): void => {
          ...
          const depthUpload = makeDepthUpload(frame);
          device.queue.writeTexture(
            { texture: depthTexture! },
            depthUpload.data,
          ...
          const colorUpload = makeColorUpload(colorFrame.width, colorFrame.height, colorFrame.data);
          device.queue.writeTexture(
            { texture: cameraTexture! },
            colorUpload.data,
          ...
          centerDepthMeters = sampleCenterDepth(frame);
        };
        ...
        const renderFrame = (): void => {
          ...
          const latest =
            now - lastUpload >= FRAME_UPLOAD_INTERVAL_MS
              ? NativeStandardCamera.getLatestLiDARDepthFrame()
              : null;
...
function makeColorUpload(
  ...
  const bytesPerRow = Math.ceil(srcRowBytes / 256) * 256;
  const swizzled = new Uint8Array(bytesPerRow * height);
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      // Native ARKit preview frames arrive as BGRA. Upload RGBA because this
      // path is more portable across the WebGPU backends used by dev clients.
      swizzled[dst] = data[src + 2] ?? 0;
      swizzled[dst + 1] = data[src + 1] ?? 0;
      swizzled[dst + 2] = data[src] ?? 0;
      swizzled[dst + 3] = data[src + 3] ?? 255;
    }
  }
  return { bytesPerRow, data: swizzled };
}
